...
A duplex batch normally arrives as two inbox PDFs: a front pass and a back pass. Pair them by consecutive filename timestamps and equal page counts, show the proposed pair, and stage both in one `stage` call with the front pass first. The command joins them, refuses unequal page counts, and reports `sheets`. Stage a single input only for a one-sided batch or an already-combined two-pass PDF.
...
Take `sheets` from the stage manifest whenever it is present and never ask the human to count paper in that case. Otherwise reuse `passes` and `sheets` from a valid inspection sidecar. Only when neither exists, show the selected filename and raw PDF page count, then ask once for confirmation of the missing capture mode and physical sheet count. Explain that the calibrated two-pass duplex routine is the safe choice for a mixed stack and that candidate blank backs will be proposed later for human approval. Inspect the scanner profile and raw page count automatically, but do not infer one-sided versus duplex solely from an odd/even page count or document content: that cannot distinguish blank backs from missing pages.
